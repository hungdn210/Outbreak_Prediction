...
def Run():
  for csv_file in constants.INPUT_DIR_LIST:
    ...
    for name, model in constants.MODELS_LIST.items():
     Y_proba, Y_pred, learned_thresholds = models.Run_single_model(model_name=name, model=model, X_train=X_train, Y_train=Y_train, X_test=X_test, Y_test=Y_test, val_size=constants.VAL_SIZE, thresholds='0.5', debug=False)
